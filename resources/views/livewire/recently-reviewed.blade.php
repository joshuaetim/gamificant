    ...
    @forelse ($recentlyReviewed as $game)
        <div class="game-info rounded-lg shadow-md bg-gray-800 px-4 py-4 flex">
            <div class="relative flex-none">
                <a href="#">
                    <img src="{{Str::replaceFirst('thumb', 'cover_big', $game['cover']['url'])}}" alt="game image" class="w-48 hover:opacity-75 transition ease-in-out duration-150">
                </a>
                @if (isset($game['rating']))
                    <div class="absolute w-16 h-16 rounded-full bg-gray-800 bottom-0 right-0" style="bottom: -20px; right: -20px">
                        <div class="font-semibold flex justify-center h-full items-center text-xs">{{round($game['rating']).'%'}}</div>
                    </div>
                @else
            ...
            <div class="lg:ml-12 ml-3">
                <a href="#" class="font-semibold text-lg mt-4 block hover:text-gray-400 leading-tight">{{$game['name']}}</a>
